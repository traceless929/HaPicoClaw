...
同时提供一个基于 Home Assistant Ingress 的控制页，内置 Home Assistant API 检测、Supervisor API 检测、`mcp-proxy` 管理和 Web Terminal，方便直接在 add-on 容器内查看配置、执行调试命令和触发辅助脚本。
...
6. 查看 add-on 日志中的上游报错与控制页里的 `mcp-proxy` 状态信息
